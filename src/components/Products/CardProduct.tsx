...
export const CardProduct: React.FC<Props> = (props: Props) => {
  return (
    <div className="min-w-[15rem] flex flex-col gap-3 shrink-0 self-stretch">
      <div
        className={`h-80 bg-neutral-200 relative flex items-center bg-[url('.${props.img}')] bg-center bg-blend-multiply bg-no-repeat bg-contain cursor-pointer`}
      >
        <div className="absolute px-6 bg-neutral-700 flex justify-center text-neutral-100 rounded-lg shadow-[0px_8px_16px_0px_rgba(0,0,0,0.04)]  py-2 text-base font-medium leading-7 inset-x-4 bottom-4">
          Add to cart
        </div>
        <SupportLikeQuestionHeartAddOutline className="text-neutral-400 p-[0.38rem] absolute right-4 top-4 rounded-full w-8 bg-white text-center font-bold shadow-[0px_8px_16px_-8px_rgba(15,15,15,0.12)]" />
